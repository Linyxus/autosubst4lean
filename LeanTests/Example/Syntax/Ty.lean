import Autosubst4Lean.Example.Debruijn
import Autosubst4Lean.Example.Syntax.CaptureSet
...
def Ty.rename {sort : TySort} : Ty sort s1 -> Rename s1 s2 -> Ty sort s2
| .top, _ => .top
...
def Ty.rename_id {sort : TySort} {T : Ty sort s} : T.rename (Rename.id) = T := by
  induction T
  case top => rfl
  case tvar => rfl
...
/-- Renaming distributes over composition of renamings. -/
theorem Ty.rename_comp {sort : TySort} {T : Ty sort s1} {f : Rename s1 s2} {g : Rename s2 s3} :
...
theorem Ty.weaken_rename_comm {sort : TySort} {T : Ty sort s1} {f : Rename s1 s2} :
...
| bound {cs : CaptureSet s} : CaptureSet.IsClosed cs -> CaptureBound.IsClosed (.bound cs)

/-- A type is closed if it contains no heap pointers. -/
inductive Ty.IsClosed : {sort : TySort} -> Ty sort s -> Prop where
-- shape types
| top : Ty.IsClosed .top
...
| cpoly {cb : CaptureBound s} : CaptureBound.IsClosed cb -> Ty.IsClosed T ->
    Ty.IsClosed (.cpoly cb T)
| unit : Ty.IsClosed .unit
| cap : Ty.IsClosed .cap
| bool : Ty.IsClosed .bool
| cell : Ty.IsClosed .cell
-- capturing types
| capt {cs : CaptureSet s} : CaptureSet.IsClosed cs -> Ty.IsClosed S -> Ty.IsClosed (.capt cs S)
-- existential types
