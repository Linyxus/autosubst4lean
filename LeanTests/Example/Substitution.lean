import Autosubst4Lean.Example.Syntax
...
def Ty.subst {sort : TySort} : Ty sort s1 -> Subst s1 s2 -> Ty sort s2
| .top, _ => .top
| .tvar x, s => s.tvar x
...
theorem Ty.weaken_subst_comm {sort : TySort} {T : Ty sort (s1 ++ K)} {σ : Subst s1 s2} :
...
theorem Ty.weaken_subst_comm_base {sort : TySort} {T : Ty sort s1} {σ : Subst s1 s2} :
...
theorem Ty.subst_comp {sort : TySort} {T : Ty sort s1} {σ1 : Subst s1 s2} {σ2 : Subst s2 s3} :
...
theorem Ty.subst_id {sort : TySort} {T : Ty sort s} :
...
/-- Substituting a substitution lifted from a renaming is the same as renaming. -/
theorem Ty.subst_asSubst {sort : TySort} {T : Ty sort s1} {f : Rename s1 s2} :
...
private theorem Ty.rename_closed_any {sort : TySort} {T : Ty sort s1} {f : Rename s1 s2}
...
def Ty.is_closed_subst {sort : TySort} {T : Ty sort s1} {σ : Subst s1 s2}
...
theorem Ty.subst_closed_inv {sort : TySort} {T : Ty sort s1} {σ : Subst s1 s2}
